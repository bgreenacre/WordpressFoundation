...
class AssetsServiceProvider extends AbstractServiceProvider {
    ...
    public function register()
    {
        ...
        $this->app['asset.register'] = function($app, $asset)
        {
            if ($asset['type'] == 'css')
            {
                wp_register_style(
        ...
        $this->app['asset.unregister'] = function($app, $name, $type)
        {
            if ($type == 'css')
            {
        ...
        $this->app['asset.enqueue'] = function($app, $name, $type)
        {
            if ($type == 'css')
            {
    ...
    public function boot()
    {
        ...
        {
            $frontAssets = array_get($this->app, 'assets.front', array());
            $loginAssets = array_get($this->app, 'assets.login', array());

            foreach (array($frontAssets, $loginAssets) as $assets)
            {
                foreach ($assets as $asset)
                {
                    if (isset($asset['replaces']))
                    {
                        ...
                        {
                            $this->app['asset.unregister']($replace, $asset['type']);
                        }
                    }

                    $this->app['asset.register']($asset);
                }
            }
        });
        ...
        {
            $adminAssets = array_get($this->app, 'assets.admin', array());
        ...
        {
            add_action($action, function() use ($context)
            {
                // Get assets array for context.
                $assets = array_get($this->app, 'assets.' . $context, array());
                ...
                foreach ($assets as $asset)
                {
                    ...
                    {
                        $this->app['asset.enqueue']($asset['handle'], $asset['type']);
                    }
                }
            });
        }
